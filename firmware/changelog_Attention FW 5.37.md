...
starting new pcb revision 3.xx with STM32 MCU
...
- Web dashboard now also available on the second LAN interface (W5500 Ethernet Controller port)
- Added **mDNS (Multicast DNS / .local hostname resolution)** support for easier network access
- type motion4sim.local in your browser to enter the dashboard, works only on systems supporting mDNS
- Introduced **Belt Torque Mode** for Actuator 7 and AASD servos (using analog 0–10 V control) *PRO feature*
- Improved  park position handling in the EtherCAD system: optimized motor standstill detection with increased tolerance
...
- Various minor fixes, adjustments, and dashboard improvements

- Web Dashboard now public on the github:
       https://github.com/motion4sim/motion4web-dashboard
- Note: Web Dashboard uses external motion4sim server , you can setup your own server and config 
        the url in the network section of the Dashboard.
	Web Dashboard is http only !! allow mixed use in the browser
	is Work in Progress use with care	
...
- note update takes 3-4 minutes
- added new rigtypes 
- factory reset linear is now vnm geometry and settings
- factory rest rotational is now mk3 settings
- added mxp rigtypes
- improved shutdown function for actuators  incl backoff factors
- added token activation support for pro & pro+ boards (no online needed)
- repaired actuator check for step/dir interface
- added  rs485 support
- small release bugfixes
- bootloader fw update, ethernet fw update
- governor fw update

Version v5.34   Attention not compatible with older boards , pcb must be > rev 3.xx !!!!!

- complete new code for stm mcu
- starting with all the features from 4.40
- adding 2nd Wind channel
- added native ethercat support for A6 servos
- added support for 2nd LAN w5500
- added WebServer for WebDashboard for native LAN w5500 will follow
-> motion4sim.local where local is supported


Atmel Boards -> for pcb revision 2.xx 

Version v4.40 compatibility update

- change cfg datasystem to cbor for compatibility to new Dashboard system
